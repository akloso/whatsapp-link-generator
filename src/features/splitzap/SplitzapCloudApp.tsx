...
async function signOutSplitzapSafely() {
...
export default function SplitzapCloudApp() {
  ...
  useEffect(() => {
    if (!hydrated || !session || !accountDataReady || initializedUser.current !== session.user.id || accountSyncing.current) return;
    ...
    const timer = window.setTimeout(() => {
      accountSyncing.current = true; setStatus('syncing'); setStatusMessage('Syncing…');
  ...
  const collaboration = {
    ...
    onGetMemberUpi: async (group: Group, memberId: string) => group.sharedId ? getSharedMemberUpi(group.sharedId, memberId) : null,
  ...
  const guestAccountAction = <button type="button" onClick={() => setAccountOpen(true)} aria-label="Guest mode · sign in free" title="Guest mode · sign in free" className="guest-profile-alert press relative grid size-9 place-items-center rounded-full border border-red-200 bg-red-50 text-red-700 shadow-sm"><UserPlus size={15} /><span aria-hidden="true" className="absolute -right-0.5 -top-0.5 grid size-4 place-items-center rounded-full border-2 border-white bg-red-500 text-[8px] font-black text-white">!</span></button>;
...
function ProfileScreen({ open, onClose, session, data, update, status, statusMessage, lastSyncedAt, profile, onProfileChanged, onOpenSecurity, onRestored }: {
  ...
  const [allowGroupUpi, setAllowGroupUpi] = useState(false);
  ...
  useEffect(() => {
    ...
    setFeedback('');
    void getSplitzapPaymentProfile().then((payment) => { setUpiId(payment.upi_id ?? ''); setAllowGroupUpi(Boolean(payment.allow_group_upi)); }).catch(() => { setUpiId(''); setAllowGroupUpi(false); });
  ...
  return <div className="fixed inset-0 z-[140] overflow-y-auto bg-[#fbfaf6] text-slate-900">{feedback ? <div role="status" aria-live="polite" className="fixed left-1/2 top-[max(0.75rem,env(safe-area-inset-top))] z-[175] w-[calc(100%-1.5rem)] max-w-[488px] -translate-x-1/2 rounded-2xl border border-[#cfe6dd] bg-white px-4 py-3 text-xs font-bold leading-5 text-[#1f665d] shadow-xl"><div className="flex items-start gap-2"><span className="grid size-5 shrink-0 place-items-center rounded-full bg-[#e7f4ef] text-[11px]">✓</span><span className="min-w-0 flex-1">{feedback}</span><button type="button" onClick={() => setFeedback('')} aria-label="Dismiss message" className="grid size-7 shrink-0 place-items-center rounded-full bg-slate-50 text-slate-500"><X size={12} /></button></div></div> : null}<div className="mx-auto min-h-[100dvh] w-full max-w-[520px] bg-[#fbfaf6] pb-[max(2rem,env(safe-area-inset-bottom))]"><header className="sticky top-0 z-10 flex items-center gap-3 border-b border-slate-200/80 bg-[#fbfaf6]/95 px-4 pb-3 pt-[max(1rem,env(safe-area-inset-top))] backdrop-blur"><button type="button" onClick={onClose} aria-label="Back" className="grid size-10 place-items-center rounded-full bg-white shadow-sm"><X size={18} /></button><div className="min-w-0 flex-1"><h2 className="text-lg font-extrabold">My Profile</h2><p className="truncate text-[11px] text-slate-500">Account, preferences and your data</p></div></header><main className="space-y-4 p-4"><section className="flex items-center gap-3 rounded-3xl bg-white p-4 shadow-sm"><span className="grid size-14 place-items-center rounded-2xl bg-[#e7f4ef] text-xl font-extrabold text-[#256f66]">{initial}</span><div className="min-w-0 flex-1"><p className="truncate text-base font-extrabold">{name.trim() || 'Splitzap user'}</p><p className="truncate text-xs text-slate-500">{session.user.email}</p></div>{status === 'offline' || status === 'error' ? <span className={`rounded-full px-2 py-1 text-[10px] font-bold ${status === 'offline' ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-700'}`}>{status === 'offline' ? 'Offline' : 'Sync issue'}</span> : null}</section>
      ...
      <ProfileSection title="Payments"><label className="block text-xs font-bold text-slate-600">UPI ID</label><input value={upiId} onChange={(event) => { const next = event.target.value.replace(/\s/g, ''); setUpiId(next); if (!next) setAllowGroupUpi(false); }} autoCapitalize="none" autoCorrect="off" spellCheck={false} inputMode="email" placeholder="name@bank" className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm outline-none focus:border-[#256f66]" /><label className="mt-3 flex items-start gap-3 rounded-xl bg-slate-50 px-3 py-3 text-xs font-bold"><input type="checkbox" checked={allowGroupUpi} disabled={!upiId.trim()} onChange={(event) => setAllowGroupUpi(event.target.checked)} className="mt-0.5 size-5 shrink-0" /><span><span className="block">Allow group members to pay me via UPI</span><span className="mt-1 block text-[10px] font-medium leading-4 text-slate-500">Only people who share an active Splitzap group with you can use this UPI ID while settling with you. It is not shown as public profile information.</span></span></label><button type="button" disabled={busy} onClick={() => void savePaymentProfile()} className="mt-3 w-full rounded-xl bg-[#e7f4ef] py-3 text-xs font-bold text-[#256f66] disabled:opacity-40">Save UPI settings</button></ProfileSection>
      ...
      <button type="button" onClick={() => { clearSplitzapExpenseDraft(session.user.id); void signOutSplitzapSafely(); }} className="w-full rounded-2xl border border-slate-200 bg-white py-3.5 text-sm font-bold text-slate-700">Sign out</button>
      ...
      <section role="dialog" aria-modal="true" aria-label={session ? 'Splitzap account' : 'Sign in to Splitzap'} className={locked ? 'relative min-h-[100dvh] w-full max-w-[520px] overflow-y-auto bg-[#fbfaf6] px-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-[max(1rem,env(safe-area-inset-top))]' : 'relative max-h-[92dvh] w-full max-w-[520px] overflow-y-auto rounded-t-[28px] bg-white px-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-2 shadow-2xl'}>
        ...
        {session ? (
          <div>
            ...
            <button type="button" onClick={() => { if (session) clearSplitzapExpenseDraft(session.user.id); void signOutSplitzapSafely().then(onClose); }} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-slate-100 px-4 py-3 text-sm font-bold text-slate-700"><LogOut size={16} /> Sign out</button>
          </div>
        ) : (
          <div>
            {guestMode ? <div className="guest-local-card mb-3 rounded-2xl p-3"><p className="text-xs font-extrabold text-red-700">Guest mode</p><p className="mt-1 text-[11px] text-red-600">Local only · Sign in free for backup + sharing.</p><div className="mt-2 grid grid-cols-3 gap-1.5 text-center text-[9px] font-bold text-red-700"><span className="rounded-lg bg-white/70 px-1 py-1.5">☁ Backup</span><span className="rounded-lg bg-white/70 px-1 py-1.5">↔ Sync</span><span className="rounded-lg bg-white/70 px-1 py-1.5">👥 Share</span></div></div> : null}
            ...
            {!guestMode && onContinueAsGuest ? <><div className="my-4 flex items-center gap-3"><span className="h-px flex-1 bg-slate-200" /><span className="text-[11px] font-semibold text-slate-400">or</span><span className="h-px flex-1 bg-slate-200" /></div><button type="button" onClick={onContinueAsGuest} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-700">Continue without login</button><p className="mt-2 text-center text-[10px] text-slate-500">Local only · no backup or sharing.</p></> : null}
          </div>
        )}
      </section>
    </div>
