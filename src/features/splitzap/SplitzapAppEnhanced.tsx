...
export default function SplitzapAppEnhanced() {
    ...
    >
      ...
      {!hydrated ? (
        ...
        <IntroPanel
          ...
          onPrimary={() =>
            data.groups.length ? setShowIntro(false) : setGroupOpen(true)
          }
        ...
        <>
          ...
          <section className="px-5 pt-6">
            <div className="mb-3 flex items-end justify-between">
              ...
              <button type="button" onClick={() => setGroupOpen(true)} className="press text-xs font-bold text-primary">
                + New group
              </button>
            </div>
...
function GroupScreen({ groupId, navigate }: { groupId: string; navigate: (view: View) => void }) {
  ...
  const nameOf = (id: string) => id === data.me ? 'You' : (group.members.find((member) => member.id === id)?.name ?? 'Someone');
          ...
          {group.members.map((member, index) => {
            ...
            return (
              <div key={member.id} className="card-soft list-enter flex items-center gap-3 p-3.5" style={{ animationDelay: `${index * 45}ms` }}>
                ...
                <p className="min-w-0 flex-1 truncate font-bold">{member.id === data.me ? 'You' : member.name}</p>
      ...
      {expense.personalItems?.length ? (
        <div className="result-section rounded-3xl border border-border bg-surface p-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="rounded-full bg-secondary px-2.5 py-1 text-xs font-bold text-secondary-foreground">{expense.personalItems.length}</span>
          </div>
              ...
              <div key={member.id} className="result-person flex items-center gap-3 rounded-2xl bg-surface-2 p-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    {money(shared, group.currency)} shared{personal > 0 ? ` + ${money(personal, group.currency)} personal` : ''}
                  </p>
                  {label ? <p className="mt-1 truncate text-[11px] font-semibold text-primary">{label}</p> : null}
                </div>
...
function ExpenseResultSheet({
  ...
  return (
    <>
      ...
      >
        <div className="mb-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{categoryOf(expense.category).emoji} {categoryOf(expense.category).label}</p>
          <h3 className="mt-1 text-2xl font-extrabold">{expense.description}</h3>
          <p className="mt-1 text-sm text-muted-foreground">Paid by {expense.paidBy === data.me ? 'You' : group.members.find((member) => member.id === expense.paidBy)?.name ?? 'Someone'}</p>
        </div>
  ...
  return (
    ...
    >
      <p className="mb-4 text-sm leading-6 text-muted-foreground">
        Add only items that belong to one person. Saved items stay here even when you close this box.
      </p>
        ...
        <div className="space-y-2">
          {items.map((item, index) => (
            <div key={item.id} className="flex items-center gap-3 rounded-2xl border border-border bg-surface-2 p-3">
              <Avatar name={nameOf(item.memberId)} size={30} />
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-bold">{item.description}</p>
                <p className="truncate text-[11px] text-muted-foreground">{nameOf(item.memberId)} · Item {index + 1}</p>
              </div>
              <p className="tabular text-sm font-extrabold">{money(item.amount, group.currency)}</p>
        ...
        <div className="mt-3 rounded-3xl border border-primary/20 bg-secondary p-3">
          ...
          <div className="grid gap-2">
            <select value={draft.memberId} onChange={(event) => setDraft({ ...draft, memberId: event.target.value })} className={`${inputClass} py-2.5 text-sm`}>
              {group.members.map((member) => (
                <option key={member.id} value={member.id}>{member.id === data.me ? 'You' : member.name}</option>
              ))}
            </select>
            ...
            <button type="button" disabled={!draft.memberId || !(Number(draft.amount) > 0)} onClick={saveDraft} className="press rounded-xl bg-primary px-3 py-2.5 text-sm font-bold text-primary-foreground disabled:opacity-40">
              Save item
            </button>
          </div>
        </div>
...
function AddExpenseSheet({
  ...
  const [labelOpen, setLabelOpen] = useState<Record<string, boolean>>(
    Object.fromEntries(Object.entries(editing?.splitLabels ?? {}).filter(([, value]) => value.trim()).map(([id]) => [id, true])),
  );
      ...
      <>
        ...
        >
          <div className="success-state py-3 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">{money(savedExpense.amount, group.currency)} has been assigned correctly.</p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {group?.members.map((member) => (
              <button key={member.id} type="button" onClick={() => setPaidBy(member.id)} className={`person-pill press flex items-center gap-2 rounded-full border px-2.5 py-1.5 text-xs font-semibold ${paidBy === member.id ? 'is-selected border-primary bg-secondary text-secondary-foreground' : 'border-border bg-surface-2 text-muted-foreground'}`}>
                <Avatar name={member.name} size={22} /> {member.id === data.me ? 'You' : member.name} {paidBy === member.id ? <Check size={12} /> : null}
              </button>
            ))}
          </div>
        </Field>
            ...
            {personalItems.length ? (
              <div className="mt-3 space-y-1.5">
                {personalItems.map((item) => {
                  const person = item.memberId === data.me ? 'You' : (group.members.find((member) => member.id === item.memberId)?.name ?? 'Someone');
                  return (
                    <div key={item.id} className="flex items-center gap-2 rounded-xl bg-surface-2 px-3 py-2 text-xs">
                      <span className="min-w-0 flex-1 truncate"><b>{person}</b> · {item.description}</span>
        ...
        <Field label={`Split shared amount · ${money(sharedTotal, group?.currency)}`}>
          ...
          <div className="divide-y divide-border overflow-hidden rounded-xl border border-border">
            {group?.members.map((member) => {
              ...
              const showLabel = mode === 'exact' && (labelOpen[member.id] || Boolean(splitLabels[member.id]));
              ...
              return (
                <div key={member.id} className="bg-surface px-3 py-2.5">
                  <div className="split-person-row flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <span className="block truncate text-sm font-semibold">{member.id === data.me ? 'You' : member.name}</span>
                  ...
                  {mode === 'exact' ? (
                    <div className="ml-[42px] mt-1.5">
                      {!showLabel ? (
                        <button type="button" onClick={() => setLabelOpen({ ...labelOpen, [member.id]: true })} className="press text-[11px] font-bold text-primary">+ Add Label (optional)</button>
                      ) : (
                        <div className="flex items-center gap-2">
                          <input
                            value={splitLabels[member.id] ?? ''}
                            onChange={(event) => setSplitLabels({ ...splitLabels, [member.id]: event.target.value })}
  ...
  return (
    ...
    >
      <div className="mb-5 rounded-3xl bg-secondary p-4">
        ...
        <p className="mt-1 text-xs leading-5 text-muted-foreground">Add only the real people in this group. No sample data is created for you.</p>
      </div>
...
function SettleSheet({
  ...
  const nameOf = (id: string) => id === data.me ? 'You' : (group.members.find((item) => item.id === id)?.name ?? 'Someone');
...
function buildExpenseShareMessage(expense: Expense, group: Group, data: SplitData) {
  const paidBy = expense.paidBy === data.me ? 'You' : (group.members.find((member) => member.id === expense.paidBy)?.name ?? 'Someone');
  const splitLines = group.members.flatMap((member) => {
    const shared = sharedShareOf(expense, member.id);
    const personal = personalShareOf(expense, member.id);
    const total = shared + personal;
    if (total <= 0.001) return [];
    const person = member.id === data.me ? 'You' : member.name;
    const label = expense.mode === 'exact' ? expense.splitLabels?.[member.id]?.trim() : '';
    const personalItems = (expense.personalItems ?? []).filter((item) => item.memberId === member.id);
    const detail = [
      label ? `  ↳ ${label}` : '',
      ...personalItems.map((item) => `  ↳ ${item.description}: ${money(item.amount, group.currency)}`),
    ].filter(Boolean);
    return [`• ${person} — ${money(total, group.currency)}`, ...detail];
  });

  const lines = [
    `🧾 ${expense.description} — ${money(expense.amount, group.currency)}`,
    `📍 ${group.name}`,
    '',
    `Paid by: ${paidBy}`,
    '',
    ...
    ...splitLines,
    '',
    `Shared amount: ${money(sharedAmountOf(expense), group.currency)}`,
  ];

  if (personalTotalOf(expense) > 0) lines.push(`Personal items: ${money(personalTotalOf(expense), group.currency)}`);
  lines.push('', 'Calculated with Splitzap');
  return lines.join('\n');
}
...
function buildGroupShareMessage(group: Group, data: SplitData) {
  ...
  const total = expenses.reduce((sum, expense) => sum + expense.amount, 0);
  const balances = groupBalances(group, data.expenses, data.settlements);
  const expenseLimit = 30;
  const shownExpenses = expenses.slice(0, expenseLimit);
  const expenseLines = shownExpenses.length
    ? shownExpenses.map((expense) => `• ${expense.description} — ${money(expense.amount, group.currency)}`)
    : ['• No expenses yet'];
  if (expenses.length > expenseLimit) expenseLines.push(`• +${expenses.length - expenseLimit} more expenses`);

  const balanceLines = group.members.map((member) => {
    const value = balances[member.id] ?? 0;
    const person = member.id === data.me ? 'You' : member.name;
    if (Math.abs(value) < 0.01) return `• ${person} — settled`;
    return `• ${person} ${value > 0 ? 'gets' : 'owes'} ${money(value, group.currency)}`;
  });

  return [
    `${group.emoji} ${group.name}`,
    `Total spent: ${money(total, group.currency)}`,
    '',
    'Expenses:',
    ...expenseLines,
    '',
