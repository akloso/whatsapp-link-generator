...
function MultiplePayersDialog({ open, onClose, group, data, total, payments, onChange, onUseSingle }: { open: boolean; onClose: () => void; group: Group; data: SplitData; total: number; payments: Record<string, number>; onChange: (value: Record<string, number>) => void; onUseSingle: () => void }) {
  const paid = Object.values(payments).reduce((sum, value) => sum + (Number(value) || 0), 0);
  const valid = Math.abs(paid - total) < 0.01 && total > 0;
  return <SheetModal open={open} onClose={onClose} title="Multiple payers" footer={<div className="grid grid-cols-2 gap-2"><button type="button" onClick={onUseSingle} className="press rounded-2xl bg-surface-2 py-3.5 text-xs font-bold">Use single payer</button><PrimaryButton onClick={onClose} disabled={!valid}>Save</PrimaryButton></div>}><div className="space-y-2">{group.members.map((member) => <div key={member.id} className="flex items-center gap-3 rounded-2xl bg-surface-2 p-3"><Avatar name={displayName(group, data, member.id)} size={30} /><span className="min-w-0 flex-1 truncate text-sm font-semibold">{displayName(group, data, member.id)}</span><input value={payments[member.id] ? String(payments[member.id]) : ''} onChange={(event) => onChange({ ...payments, [member.id]: Number(event.target.value.replace(/[^0-9.]/g, '')) || 0 })} inputMode="decimal" placeholder="0" className="tabular w-24 rounded-xl border border-border bg-surface px-2 py-2 text-right text-sm font-bold" /></div>)}</div><div className={`mt-3 rounded-2xl px-3 py-2 text-xs font-bold ${valid ? 'bg-secondary text-primary' : 'bg-surface-2 text-negative'}`}>Paid {money(paid, group.currency)} / {money(total, group.currency)} {valid ? '✓' : ''}</div></SheetModal>;
}
...
function AddExpenseSheet({ open, onClose, data, update, defaultGroupId, editing }: { open: boolean; onClose: () => void; data: SplitData; update: (fn: (data: SplitData) => SplitData) => void; defaultGroupId?: string; editing?: Expense | null }) {
  ...
  const [multiPayer, setMultiPayer] = useState(Object.keys(initialPayments).length > 1);
  ...
  const paidTotal = multiPayer ? Object.values(payments).reduce((sum, value) => sum + (Number(value) || 0), 0) : grandTotal;
  const payerValid = !multiPayer || Math.abs(paidTotal - grandTotal) < 0.01;
  ...
  const save = () => {
    ...
    const cleanPayments = multiPayer ? Object.fromEntries(Object.entries(payments).filter(([, value]) => Number(value) > 0)) : { [paidBy]: grandTotal };
    const primaryPayer = Object.keys(cleanPayments)[0] ?? paidBy;
    const payload: Expense = {
      id: editing?.id ?? uid(), groupId: group.id, description: description.trim(), amount: grandTotal, baseAmount: baseTotal, paidBy: primaryPayer, payments: cleanPayments,
...
function buildGroupShareMessage(group: Group, data: SplitData) {
  ...
  const lines = debts.length ? debts.map((debt) => `• ${displayName(group, data, debt.from)} → ${displayName(group, data, debt.to)}: ${shareMoney(debt.amount, group.currency)}`) : ['• Everyone is settled'];
  return ['💸 Splitzap', `${group.name} · ${shareMoney(total, group.currency)}`, '', 'Settlement', ...lines].join('\n');
}
