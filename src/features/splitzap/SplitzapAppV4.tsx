...
function ActivityScreen({ navigate, collaboration }: { navigate: (view: View) => void; collaboration?: SplitzapCollaboration }) {
  ...
  return <AppShell onAdd={() => setQuickOpen(true)} view={{ name: 'activity' }} navigate={navigate}>
    ...
    <QuickActionsSheet open={quickOpen} onClose={() => setQuickOpen(false)} onAddExpense={() => activeGroups.length ? setAddOpen(true) : setGroupOpen(true)} onNewGroup={() => setGroupOpen(true)} onRecordPayment={() => activeGroups.length === 1 ? setPaymentGroupId(activeGroups[0]!.id) : activeGroups.length ? setPaymentPickerOpen(true) : setGroupOpen(true)} />
    <GroupPickerSheet open={paymentPickerOpen} onClose={() => setPaymentPickerOpen(false)} groups={activeGroups} onPick={setPaymentGroupId} />
...
function GroupScreen({ groupId, navigate, collaboration, resumeSettlement = false, onSettlementResumeConsumed }: { groupId: string; navigate: (view: View) => void; collaboration?: SplitzapCollaboration; resumeSettlement?: boolean; onSettlementResumeConsumed?: () => void }) {
  ...
  return <AppShell onAdd={() => setQuickOpen(true)} view={{ name: 'group', groupId }} navigate={navigate}>
    <Header title={`${group.emoji} ${group.name}`} subtitle={`${group.members.length} people${group.sharedId ? ' · Shared live' : ''}${isArchived ? ' · Archived' : ''}`} back={<button type="button" onClick={() => navigate({ name: 'home' })} className="press grid size-9 place-items-center rounded-full bg-surface-2"><ArrowLeft size={18} /></button>} right={<div className="relative flex items-center gap-1"><button type="button" onClick={() => setShareOpen(true)} aria-label="Share group summary" title="Share group" className="press grid size-9 place-items-center rounded-full bg-secondary text-primary"><Share2 size={16} /></button><button type="button" onClick={() => setMenuOpen((value) => !value)} aria-label="Group options" className="press grid size-9 place-items-center rounded-full bg-surface-2"><MoreHorizontal size={18} /></button>{menuOpen ? <><button type="button" aria-label="Close group menu" onClick={() => setMenuOpen(false)} className="fixed inset-0 z-30 cursor-default bg-transparent" /><div className="absolute right-0 top-11 z-40 w-52 overflow-hidden rounded-2xl border border-border bg-surface p-1.5 shadow-xl">{!isArchived ? <button type="button" onClick={() => { setMenuOpen(false); setEditGroupOpen(true); }} className="press flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-xs font-bold"><Pencil size={14} /> Edit group</button> : null}{group.sharedId ? <button type="button" onClick={() => { setMenuOpen(false); collaboration?.onManageMembers?.(group.id); }} className="press flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-xs font-bold"><Users size={14} /> Manage members</button> : null}{!isArchived ? <button type="button" onClick={() => { setMenuOpen(false); collaboration?.onInviteGroup(group.id); }} className="press flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-xs font-bold"><Share2 size={14} /> {group.sharedId ? 'Invite people' : 'Share group live'}</button> : null}{(!group.sharedId || isOwner) ? <button type="button" onClick={() => void archive()} className="press flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-xs font-bold"><Archive size={14} /> {isArchived ? 'Unarchive group' : 'Archive group'}</button> : null}<button type="button" onClick={() => { setMenuOpen(false); setDuplicateOpen(true); }} className="press flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-xs font-bold"><Copy size={14} /> Duplicate group</button><div className="my-1 border-t border-border" /><button type="button" onClick={() => { setMenuOpen(false); setDeleteMode(group.sharedId && isOwner ? 'everyone' : 'self'); setTransferMemberId(''); setDeleteOpen(true); }} className="press flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-xs font-bold text-negative"><Trash2 size={14} /> {group.sharedId && !isOwner ? 'Leave group' : 'Delete / remove group'}</button></div></> : null}</div>} />
...
function BalancesTab({ group, data, balances, update, newMember, setNewMember }: { group: Group; data: SplitData; balances: Record<string, number>; update: (fn: (data: SplitData) => SplitData) => void; newMember: string; setNewMember: (value: string) => void }) {
  return <section className="space-y-2 px-5 pt-3">{group.members.map((member, index) => { const value = balances[member.id] ?? 0; return <div key={member.id} className="list-enter flex items-center gap-3 rounded-2xl border border-border bg-surface p-3.5" style={{ animationDelay: `${index * 35}ms` }}><Avatar name={displayName(group, data, member.id)} size={38} /><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{displayName(group, data, member.id)}</p><p className="text-[10px] text-muted-foreground">{member.id === memberIdFor(group, data) ? 'You' : 'Group member'}</p></div><p className={`tabular text-sm font-extrabold ${Math.abs(value) < 0.01 ? 'text-muted-foreground' : value > 0 ? 'text-positive' : 'text-negative'}`}>{Math.abs(value) < 0.01 ? 'Settled' : `${value > 0 ? 'Gets ' : 'Owes '}${money(Math.abs(value), group.currency)}`}</p></div>; })}{(group.status ?? group.sharedStatus ?? 'active') === 'archived' ? null : !group.sharedId ? <div className="rounded-2xl border border-dashed border-border bg-surface p-3.5"><p className="mb-2 flex items-center gap-1.5 text-xs font-bold text-muted-foreground"><UserPlus size={13} /> Add someone</p><div className="flex gap-2"><input value={newMember} onChange={(event) => setNewMember(event.target.value)} placeholder="Name" className="w-full rounded-xl border border-border bg-surface-2 px-3 py-2.5 text-sm outline-none focus:border-primary" /><button type="button" disabled={!newMember.trim()} onClick={() => { const name = newMember.trim(); update((current) => ({ ...current, groups: current.groups.map((item) => item.id === group.id ? { ...item, members: [...item.members, { id: uid(), name }] } : item) })); setNewMember(''); }} className="press shrink-0 rounded-xl bg-primary px-4 text-sm font-bold text-primary-foreground disabled:opacity-40">Add</button></div></div> : <button type="button" className="w-full rounded-2xl bg-surface-2 px-3 py-3 text-left text-xs font-semibold text-muted-foreground" onClick={() => undefined}>Shared member access is managed from ··· → Manage members.</button>}</section>;
}
...
function ExpenseResultSheet({ open, onClose, expense, group, data, onEdit, onDelete, onSettle }: { open: boolean; onClose: () => void; expense: Expense | null; group: Group; data: SplitData; onEdit?: () => void; onDelete?: () => void; onSettle?: () => void }) {
  ...
  return <><SheetModal open={open} onClose={onClose} title="Expense result" footer={<div className={`grid gap-2 ${onEdit ? 'grid-cols-3' : 'grid-cols-2'}`}>{onEdit ? <button type="button" onClick={onEdit} className="press rounded-2xl bg-surface-2 py-3.5 text-xs font-bold">Edit</button> : null}<button type="button" onClick={() => setShareOpen(true)} className="press flex items-center justify-center gap-1 rounded-2xl bg-secondary py-3.5 text-xs font-bold text-primary"><Share2 size={14} /> Share</button><button type="button" onClick={onClose} className="press rounded-2xl bg-primary py-3.5 text-xs font-bold text-primary-foreground">Done</button></div>}><ExpenseBreakdown expense={expense} group={group} data={data} onHistory={() => setHistoryOpen(true)} />{onSettle ? <button type="button" onClick={onSettle} title="Uses the current simplified group balance" className="press mt-3 flex w-full items-center justify-center gap-1.5 rounded-xl bg-secondary py-3 text-xs font-bold text-primary">Settle related balance <Info size={11} /></button> : null}{onDelete ? <button type="button" onClick={onDelete} className="press mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-negative/5 py-3 text-xs font-bold text-negative"><Trash2 size={14} /> Delete expense</button> : null}</SheetModal><ShareDialog open={shareOpen} onClose={() => setShareOpen(false)} title="Share expense" message={buildExpenseShareMessage(expense, group, data)} /><HistoryDialog open={historyOpen} onClose={() => setHistoryOpen(false)} expense={expense} data={data} /></>;
}
...
function ReceiptScanner({ open, onClose, data, onParseReceipt, onUse }: { open: boolean; onClose: () => void; data: SplitData; onParseReceipt?: (text: string) => Promise<SplitzapReceiptIntelligence>; onUse: (seed: ScanExpenseSeed) => void }) {
  ...
  useEffect(() => {
    if (open && !groupId && availableGroups[0]) setGroupId(availableGroups[0].id);
  }, [open, groupId, availableGroups]);
  ...
  return <SheetModal open={open} onClose={close} title="Scan a bill" footer={status === 'review' ? <div className="grid grid-cols-[.8fr_1.2fr] gap-2"><button type="button" onClick={reset} className="press rounded-2xl bg-surface-2 py-3.5 text-xs font-bold">Scan another</button><PrimaryButton onClick={useBill} disabled={!group || baseTotal <= 0}>Use this bill</PrimaryButton></div> : undefined}>
    {!availableGroups.length ? <div className="rounded-3xl bg-surface-2 p-6 text-center"><Camera size={24} className="mx-auto text-primary" /><p className="mt-3 font-extrabold">Create a group first</p><p className="mt-1 text-xs text-muted-foreground">A scanned bill needs a group so items can be shared or assigned.</p></div> : <>
...
function NewGroupSheet({ open, onClose, data, update, onCreated, onJoinGroup }: { open: boolean; onClose: () => void; data: SplitData; update: (fn: (data: SplitData) => SplitData) => void; onCreated?: (groupId: string) => void; onJoinGroup?: () => void }) {
  ...
  const creator = data.myName?.trim() ?? '';
