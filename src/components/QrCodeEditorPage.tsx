...
type ModuleStyle = 'classic' | 'square' | 'rounded' | 'dots' | 'smooth';
...
const chipButton = (selected: boolean) => `${baseInteractive} ${selected ? 'border-emerald-300 bg-emerald-50 text-emerald-900 shadow-sm' : 'border-slate-200 bg-white text-slate-700 hover:border-slate-300 hover:bg-slate-50'}`;
...
export default function QrCodeEditorPage() {
  ...
  const qr = finalLink ? `https://api.qrserver.com/v1/create-qr-code/?size=1024x1024&ecc=H&color=000000&bgcolor=ffffff&data=${encodeURIComponent(finalLink)}` : '';
  ...
  const drawStyledQr = async (ctx: CanvasRenderingContext2D, x: number, y: number, sizePx: number) => {
    ...
    for (let r = 0; r < m; r++) for (let c = 0; c < m; c++) {
      ...
      if (moduleStyle === 'dots') { ctx.beginPath(); ctx.arc(px + cell / 2, py + cell / 2, cell * 0.3, 0, Math.PI * 2); ctx.fill(); }
      if (moduleStyle === 'smooth') { ctx.beginPath(); ctx.roundRect(px + cell * 0.03, py + cell * 0.03, cell * 0.94, cell * 0.94, cell * 0.4); ctx.fill(); }
    }
  ...
  const renderCanvas = async () => {
    ...
    const pad = Math.round(Math.min(c.width, c.height) * 0.05); const w = c.width - pad * 2; const h = c.height - pad * 2;
    ctx.fillStyle = '#fff'; ctx.strokeStyle = '#e5e7eb'; ctx.lineWidth = 2; ctx.beginPath(); ctx.roundRect(pad, pad, w, h, 24); ctx.fill(); ctx.stroke();
    const headerH = style === 'plain' ? 0 : Math.round(h * 0.18);
    if (style !== 'plain') {
    ...
    const qs = Math.round(Math.min(c.width, c.height) * 0.62);
    const qx = (c.width - qs) / 2;
    const textZone = style === 'plain' ? 0 : Math.round(h * 0.20);
  ...
  return <main className='min-h-screen bg-white py-8 sm:py-12'><div className='mx-auto max-w-7xl px-4 lg:px-6'><header className='mb-6 rounded-3xl border border-slate-200 bg-white p-5 sm:mb-8 sm:p-7'><p className='mb-2 inline-flex items-center rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-emerald-700'>Zapora Studio</p><h1 className='text-2xl font-semibold tracking-tight text-slate-900 sm:text-4xl'>QR Code Editor</h1></header>
    <section className='grid gap-5 xl:grid-cols-[1fr_1fr]'><div className='rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6 min-h-[760px]'><div className='mb-5 grid grid-cols-3 gap-2 rounded-2xl bg-slate-100 p-1.5'>{([{ id: 'content', label: 'Content' }, { id: 'design', label: 'Design' }, { id: 'appearance', label: 'Appearance' }] as { id: WorkspaceTab; label: string }[]).map((item) => <button key={item.id} onClick={() => setTab(item.id)} className={`${baseInteractive} border-0 px-3 py-2.5 text-sm ${tab === item.id ? 'bg-white text-slate-950 shadow ring-1 ring-emerald-200' : 'bg-transparent text-slate-600 hover:bg-white/80'}`}>{item.label}</button>)}</div>
      ...
      <div className='space-y-4 xl:sticky xl:top-5 xl:self-start'><section className='rounded-3xl border border-slate-200 bg-gradient-to-b from-white to-slate-50 p-4 shadow-sm sm:p-5'><div className='mb-4 flex items-center justify-between'><h2 className='inline-flex items-center gap-2 text-sm font-semibold uppercase tracking-wide text-slate-700'><Sparkles className='h-4 w-4 text-emerald-500' />Live Canvas</h2><span className='rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700'>{exportSizes[size].label}</span></div>{valid && previewQr ? <div className='grid min-h-[460px] place-items-center rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-100 to-white p-5'><div className='w-full max-w-md rounded-3xl border border-slate-200 bg-white p-4 shadow-lg'><img src={previewQr} className='w-full rounded-2xl border border-slate-200 bg-white p-2' alt='preview' /></div></div> : <div className='flex min-h-[440px] flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 text-center text-slate-700'><Sparkles className='mb-2 h-5 w-5 text-emerald-500' /><p className='font-semibold'>Start with a link to preview and export</p></div>}</section><section className='rounded-3xl border border-slate-200 bg-white p-5 shadow-sm ring-1 ring-slate-100'><div className='mb-3 flex items-center gap-2 text-slate-800'><ImageDown className='h-4 w-4 text-emerald-700' /><h3 className='text-sm font-semibold uppercase tracking-wide text-slate-500'>Finalize output</h3></div><div className='mb-4 grid grid-cols-2 gap-2'>{(Object.keys(exportSizes) as ExportSizeId[]).map((k) => <button key={k} onClick={() => setSize(k)} className={`${chipButton(size === k)} text-left`}>{exportSizes[k].label} <span className='text-[10px] text-slate-500'>— {exportSizes[k].ratio}</span></button>)}</div><div className='grid grid-cols-3 gap-2'><button onClick={exportPNG} disabled={!valid} className={`${baseInteractive} border-transparent bg-gradient-to-r from-emerald-600 to-green-500 text-white`}>PNG</button><button onClick={exportSVG} disabled={!valid} className={chipButton(false)}>SVG</button><button onClick={exportPDF} disabled={!valid} className={chipButton(false)}>PDF</button></div></section></div></section></div></main>;
}
