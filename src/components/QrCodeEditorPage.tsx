...
export default function QrCodeEditorPage() {
  ...
  const renderCanvas = async () => {
    ...
    const headerH = style === 'plain' ? 0 : Math.round(h * 0.2);
    if (style !== 'plain') {
    ...
    const textZone = style === 'plain' ? Math.round(h * 0.04) : Math.round(h * 0.09);
    ...
    const qs = Math.round(Math.min(Math.min(c.width, c.height) * 0.64, maxQrByHeight));
